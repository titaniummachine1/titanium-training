...
Import new games into the canonical store after pool runs:
  python -m titanium_training.store.cli import-legacy-games training/data/all_games.db
...
from tools.datagen.datagen import DB_PATH, insert_single_game, insert_single_game_idempotent, validate_game
from color_rotation import claim_local_game
from tools.operations.opponent_curriculum import claim_game, load_state as load_curriculum, record_result
from tools.maintenance.manifest import format_scoreboard, format_scoreboard_compact, load_manifest, lookup_prior_wins, save_manifest, update_matchup
from tools.operations.swiss_tournament import (
    KA_TIME_CONTROLS,
    MAX_KA_PER_TC,
    OUR_TIME_CONTROLS,
    max_remote_parallel,
...
class CoordinatorHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/pool-status":
            from tools.operations.swiss_tournament import eligible_pairings
            with _lock:
